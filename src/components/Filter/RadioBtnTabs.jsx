...
const RadioBtnTabs = ({ name, values, field, setFieldFunc, onChangeFunc = () => { }, ...props }) => {
  function onChangeField(event) {
    setFieldFunc(Number(event.target.value));
      ...
      <ul className="RadioBtnTabs_list">
        {
          Object.entries(values).map(item => (
            <li key={item[0]} className="RadioBtnTab RadioBtnTabs_item">
              <input className="RadioBtnTab_input" type="radio" name={name} value={item[0]} id={`${name}_${item[0]}`} checked={field === Number(item[0]) ? true : false} onChange={onChangeField} />
